 ...
 */
    ...
    public class Robot extends Item {
       ...
       public Robot(int x, int y, int dir, int beepers)
      {
         ...
         if(WorldManager.getCurrent() == null)
         {
         ...
         WorldManager.getCurrent().addRobot(this);
      }
       ...
       public void putBeeper()
      {
         ...
         WorldManager.getCurrent().putBeepers(x, y, 1);
       ...
       public void pickBeeper()
      {
         ...
         if(!WorldManager.getCurrent().checkBeepers(x, y))
         {
      ...
      {
         return WorldManager.getCurrent().checkBeepers(x, y);
      }
   
       public boolean nextToARobot()
      {
         return WorldManager.getCurrent().isNextToARobot(this, x, y);
      }
       ...
       private boolean isClear(int direction)
      {
         ...
         switch(direction)
         {
            case Display.NORTH:
            case Display.SOUTH:
               return !WorldManager.getCurrent().checkWall(c.x, c.y, Display.HORIZONTAL);
            case Display.EAST:
            case Display.WEST:
            default:
               return !WorldManager.getCurrent().checkWall(c.x, c.y, Display.VERTICAL);
         }
      }
   
       public void explode()
      {
         WorldManager.getCurrent().removeRobot(this);
      }
